...
def get_legendre_hypergeometric(n, m, theta):
    """ 
    Calculate Schmidt Seminormalized Legendre functions of degree n, order m, at colatutide theta

    n -- array-like degrees n
    m -- array-like degrees m (same size as n!)
    theta -- array-like colatitude in degrees

    outputs:
    P, dP: shape (n.size, theta.size) - first dimension corresponds to (n, m), and second to theta
    ...
    x = np.cos(theta.flatten() * np.pi  / 180).reshape((1, -1))

    # calculate Schmidt normalization:
    Knm = np.ones(n.size)
    iii = (m != 0) 
    Knm[iii] = np.sqrt(2) / (2**m[iii] * factorial(m[iii])) * \
               np.sqrt(gamma(n[iii] + m[iii] + 1) / gamma(n[iii] - m[iii] + 1))

    # reshape arrays for broadcasting
    m, n, Knm = m.reshape((-1, 1)), n.reshape((-1, 1)), Knm.reshape((-1, 1))

    # calculate hypergeometic function
    a, b, c = m - n, n + m + 1, 1 + m
    z = (1 - x) / 2

    F      = hyp2f1(a    , b    , c    , z)
    dF_dz  = hyp2f1(a + 1, b + 1, c + 1, z) * a * b / c
    ...
    P  = Knm * (F * sintheta ** m)
    dP = Knm * (F * sintheta ** (m - 1) * costheta * m + dF_dz * sintheta ** (m + 1) / 2)
...
if __name__ == '__main__':
    ...
    print ('testing legendre calculations')
    n, m = [1, 90, 3, 7], [1, 52, 2, 5]
    keys = list(zip(n, m))

    theta = np.linspace(1e-3, 180-1e-4, 1000)

    P1, dP1 = np.split(get_legendre(np.max(n), np.max(m), theta, keys = keys).T, 2, axis = 0)
    P2, dP2 = get_legendre_hypergeometric(n, m, theta)

    assert(np.all(np.isclose( P1 -  P2, 0)))
    assert(np.all(np.isclose(dP1 - dP2, 0)))
